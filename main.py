...
ds = ray.data.read_binary_files("zips", partition_filter=FileExtensionFilter("pdf"))
...
class Embed:
    ...
    def __call__(self, text_batch: List[str]):
        ...
        embeddings = self.transformer.encode(
            text_batch,
            batch_size=200,  # Large batch size to maximize GPU utilization.
            device="cuda",
        ).tolist()
...
ds = ds.map_batches(
    ...
    compute=ray.data.ActorPoolStrategy(min_size=2, max_size=20),  # I have 20 GPUs in my cluster
    num_gpus=2,  # 1 GPU for each actor.
)
